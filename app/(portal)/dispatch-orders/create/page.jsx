...
export default function CreateDispatchOrderPage() {
  ...
  const [currentStage, setCurrentStage] = useState('creating');
  ...
  const handleSubmit = async (formData, productImages) => {
    ...
    try {
      // Stage 1: Create dispatch order (0-20%)
      setOverallProgress(10);
      const dispatchOrder = await createDispatchOrder(formData);
      const dispatchOrderId = dispatchOrder._id || dispatchOrder.id;
      setOverallProgress(20);

      // Stage 2: Upload images (20-100%)
      if (productImages && Object.keys(productImages).length > 0 && dispatchOrderId) {
              ...
              try {
                await uploadDispatchOrderItemImage(
                  ...
                  (progress) => {
                    ...
                    // Update overall progress: 20% (order created) + (currentImageIndex * progressPerImage) + (progress * progressPerImage / 100)
        ...
        if (imageUploadErrors.length > 0) {
          ...
          setTimeout(() => {
            ...
            const userErrorMessage = `Dispatch order created, but ${imageUploadErrors.length} image upload(s) failed. You can try uploading the images again from the edit page.`;
      ...
      setShowProgressModal(false);
      handleApiError(err, "Failed to create dispatch order. Please try again.");
      setIsSaving(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Fill in the product information, quantities, and box configurations.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <DispatchOrderForm
