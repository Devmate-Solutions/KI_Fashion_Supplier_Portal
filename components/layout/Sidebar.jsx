    ...
    <aside className="hidden w-64 flex-col border-r border-app-border bg-white md:flex">
      ...
      <div className="flex flex-1 flex-col justify-between overflow-y-auto p-4">
        <nav className="flex flex-col gap-1" aria-label="Main navigation">
          <p className="mb-2 px-2 text-[10px] font-semibold uppercase tracking-wider text-slate-400">Main Menu</p>
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <Link
                ...
                className={clsx(
                  "flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-app-accent focus:ring-offset-2",
                  isActive
                    ? "bg-app-accent/10 text-app-accent shadow-sm shadow-app-accent/5 font-semibold"
                    : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"
                )}
              >
                <Icon className={clsx("h-4.5 w-4.5 shrink-0", isActive ? "text-app-accent" : "text-slate-400")} />
                {item.label}
              </Link>
            );
          })}
        ...
        <div className="mt-auto space-y-4 shrink-0">
          <div className="rounded-lg bg-slate-50 p-4 border border-slate-100">
            ...
            <a
              ...
              className="mt-3 block text-center rounded-lg bg-white py-1.5 text-[11px] font-semibold text-slate-700 shadow-sm ring-1 ring-slate-200 hover:bg-slate-50 transition-colors focus:outline-none focus:ring-2 focus:ring-app-accent focus:ring-offset-2"
            >
              Contact Support
            </a>
          </div>
