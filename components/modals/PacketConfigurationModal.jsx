...
export default function PacketConfigurationModal({
  ...
  useEffect(() => {
    if (isOpen && activeItem) {
      const initialPackets = activeItem.packets || [];
      
      if (initialPackets.length > 0) {
        const isLoose = initialPackets.some(p => p.isLoose);
        setConfigMode(isLoose ? "loose" : "packets");
        
        setPackets(
          initialPackets.map((p, pIdx) => ({
            packetNumber: p.packetNumber || pIdx + 1,
            totalItems: parseInt(p.totalItems) || 0,
            composition: (p.composition || []).map(comp => ({
              size: String(comp.size || '').trim(),
  ...
  const handleCompositionChange = (packetIndex, composition) => {
    ...
    if (expectedTotal && newGrandTotal > expectedTotal) {
      showError(
        `You cannot configure more than ${expectedTotal} items for this product.`
      );
      return;
    }
  ...
  const handleSave = () => {
    ...
    if (expectedTotal && totalItemsInPackets > expectedTotal) {
      showError(
        `Configured items (${totalItemsInPackets}) cannot exceed quantity (${expectedTotal}).`
      );
      return;
    }
      ...
      <div className="space-y-4">
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-md p-3 flex items-center justify-between">
          <div className="flex items-center gap-3 text-slate-800">
            ...
            <div className="flex flex-col">
              <span className="font-medium text-sm">{activeItem.productName}</span>
              {activeItem.productCode && (
